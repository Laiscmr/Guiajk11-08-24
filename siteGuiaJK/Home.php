...
function getProfilePicture($conexao, $userId) {
    // Query the database to retrieve the profile picture
    $stmt = $conexao->prepare("SELECT foto FROM usuario WHERE id =?");
    ...
    $result = $stmt->get_result();
    if (!$result) {
        echo "Erro ao obter o resultado: " . $conexao->error;
        return null;
    }
    $row = $result->fetch_assoc();
    if (!$row) {
        echo "Erro ao obter a linha: " . $conexao->error;
        return null;
    }
    return $row['foto'];
}
...
<nav>
       ...
       <img id="imgPerfil" src="data:<?php echo $profilePicture['type'];?>;base64,<?php echo base64_encode($profilePicture['data']);?>" />
