<!DOCTYPE html>
<html>
<head>
    <title>Tela de Cadastro</title>
    <link rel="stylesheet" type="text/css" href="css\Cadastro.css">
    <link rel="stylesheet" type="text/css" href="css\erro.css">
    <script src="js\sweetalert2.js"></script>
    <script src="js\custom.js"></script>
</head>
<body>
<?php
include_once("conexao.php");
if(isset($_POST["submit"])){
    $nome = $_POST["nome"];
    $email = $_POST["email"];
    $senha = $_POST["senha"];
    $aluno = isset($_POST['aluno'])? 1 : 0;
    $data_nasc = $_POST["data_nascimento"];
    $confirma_senha = $_POST["confirmar_senha"];

    // Verifique se o e-mail já existe
    $checkEmail = mysqli_query($conexao, "SELECT * FROM usuario WHERE email='$email'");
    if(mysqli_num_rows($checkEmail) > 0){
       ?>
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                Swal.fire({
                    icon: 'error',
                    title: 'Erro',
                    text: 'Este e-mail já está cadastrado.',
                    confirmButtonColor: '#3085d6',
                    confirmButtonText: 'Fechar'
                });
            });
        </script>
        <?php
    } else if ($senha!= $confirma_senha){
       ?>
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                Swal.fire({
                    icon: 'error',
                    title: 'Erro',
                    text: 'As senhas não conferem.',
                    confirmButtonColor: '#3085d6',
                    confirmButtonText: 'Fechar'
                });
            });
        </script>
        <?php
    } else {
        // Trata o upload de imagem
       // Checa se o upload foi realizado com sucesso
if(isset($_FILES['foto']) && $_FILES['foto']['error'] == 0){
    $arquivo_tmp = $_FILES['foto']['tmp_name'];
    $nome_foto = $_FILES['foto']['name'];

    // Pega a extensão do arquivo
    $extensao = strtolower(pathinfo($nome_foto, PATHINFO_EXTENSION));

    // Converte o nome em um nome único
    $novoNome = uniqid();
    $caminho_final = "uploads/". $novoNome. ".". $extensao;

    if (!file_exists('uploads')) {
        mkdir('uploads', 0777, true);
    }

    // Checa se o upload foi realizado com sucesso
    if(move_uploaded_file($arquivo_tmp, $caminho_final)){
        echo "Upload realizado com sucesso.<br>";
    } else {
        echo "Não foi possível realizar o upload.<br>";
    }
} else {
    echo "Você não enviou nenhum arquivo.<br>";
}

        // Insere os dados no banco de dados
        $result = mysqli_query($conexao, "INSERT INTO usuario (nome,email,aluno,data_nasc,senha,foto) 
        VALUES ('$nome','$email','$aluno','$data_nasc','$senha','$caminho_final')");
       ?>
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                Swal.fire({
                    icon: 'Success',
                    title: 'Sucesso',
                    text: 'Cadastrado com sucesso!',
                    confirmButtonColor: '#3085d6',
                    confirmButtonText: 'Fechar'
                }).then(() => {
                    window.location.href = 'TelaLogin.php';
                });
            });
        </script>
        <?php
    }
}
?>
       

       <form id="cad-usuario-form" action="TelaCadastro.php" method="post" enctype="multipart/form-data">

        <h1 id="cad">CADASTRO</h1>
        
            <label for="nome"></label><br>
            <input type="text"placeholder="Insira o seu nome" id="nome" name="nome" required><img id="User" src="data:image/png;base64,"/><br>
             <label for="email"></label><br>
            <input type="email"placeholder="Insira o seu E-mail" id="email" name="email" required><img id="imgEmail" src="data:image/png;base64,"/><br>

            <label for="senha"></label><br>
            <input type="password"placeholder="Insira o sua senha" id="senha" name="senha" required><img id="imgSenha" src="data:image/png;base64,"/><br>

            <label for="confirmar_senha"></label><br>
            <input type="password"placeholder="Confirme a sua senha" id="confirmar_senha" name="confirmar_senha" required><img id="imgCSenha" src="data:image/png;base64,"/><br>

            <br><label for="data_nascimento"></label><p id="roxo">Data de Nascimento:</p>
            <input type="date" id="data_nascimento" name="data_nascimento" required><br>
            <br><label for="aluno"></label><p id="roxo">É aluno da rede FAETEC?</p>

<input type="checkbox" id="sim" name="aluno" value="sim"><br>
<label for="sim"></label><p id="roxo">Sim</p><br>



<!--... outros campos do formulário... -->

<p id="roxo">Insira uma imagem de perfil (opcional):</p>
<input type="file" id="foto" name="foto" accept="image/*"><br>
<input type="submit" name="submit" value="Cadastrar"><br>
   
</form>
       
            
        
     
    </body>
</html>
